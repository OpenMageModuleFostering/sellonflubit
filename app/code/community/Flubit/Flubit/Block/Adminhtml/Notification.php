...
class Flubit_Flubit_Block_Adminhtml_Notification extends Mage_Adminhtml_Block_Template {
	...
	function checkFailedOrders() {
		...
		// perhaps limit by date? chosen 7
        //$coll = Mage::getModel('flubitlog/flubitlog')->getCollection()->addFieldToFilter('created_at', array('gt' => date("Y-m-d H:i:s", strtotime('-7 day'))));
        $coll = Mage::getModel('flubitlog/flubitlog')->getCollection()->addFieldToFilter('datetime', array('gt' => date("Y-m-d H:i:s", strtotime('-3 day'))));
        if ($coll->getSize()) {
            foreach ($coll as $logs) {
                if (($logs->getData('action')=="7") && ($logs->getData('level')=="2")) {
					$failedOrders[] = $logs->getData('flubitlog_id');
                }
            }
        }
